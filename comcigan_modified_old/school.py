...
# school.py 27번 줄 다음에 디버깅 코드 추가
comci_resp = get(f"{URL}/st")
comci_resp.encoding = "EUC-KR"

print(f"[DEBUG] Status Code: {comci_resp.status_code}")
print(f"[DEBUG] Response Length: {len(comci_resp.text)}")
print(f"[DEBUG] First 500 chars: {comci_resp.text[:500]}")

comcigan_html = BeautifulSoup(comci_resp.text, "lxml")
scripts = comcigan_html.find_all("script")
print(f"[DEBUG] Script tags found: {len(scripts)}")
if len(scripts) > 1:
    print(f"[DEBUG] Script[1] length: {len(scripts[1].contents)}")
    if scripts[1].contents:
        print(f"[DEBUG] Script[1] first 200 chars: {scripts[1].contents[0][:200]}")

URL = "http://222.106.100.23:4082"

# school.py 파일 수정 (49-55번 줄 부근)

comcigan_html = BeautifulSoup(comci_resp.text, "lxml")
scripts = comcigan_html.find_all("script")
print(f"[DEBUG] Script tags found: {len(scripts)}")

# 모든 script에서 route 패턴 찾기
route = None
script_content = None
for idx, sc in enumerate(scripts):
    if sc.contents:
        content = sc.contents[0]
        match = routereg.search(content)
        if match:
            print(f"[DEBUG] Route pattern found in script[{idx}]")
            route = match.group(0)
            script_content = content
            break

if not route:
    print("[ERROR] Route pattern not found in any script tag")
    print("[DEBUG] Dumping all scripts:")
    for idx, sc in enumerate(scripts):
        if sc.contents:
            print(f"\n--- Script[{idx}] (first 300 chars) ---")
            print(sc.contents[0][:300])
    raise ValueError("컴시간 서버 응답 구조 변경 - route 패턴 찾을 수 없음")
...
class School:
    ...
    sccode: int
    _timeurl: str
    _week_data: List[List[List[List[Tuple[str, str, str, int]]]]]  # teacherId 추가
    ...
    def __init__(self, name: str):
        ...
        self._week_data = [[[[("", "", "", 0)]]]]  # teacherId 기본값 0
        self.refresh()
