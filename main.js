...
function getDefaultBounds() {
  return { x: 223, y: 0, width: 1697, height: 1032 };
}
...
ipcMain.handle('start-comcigan-server', async () => {
  try {
    const pythonProcess = spawn('python', ['comcigan_api.py'], {
      cwd: __dirname,
      detached: true,
      stdio: 'ignore'
    });
    ...
    console.error('Python 서버 실행 실패:', error);
    return false;
  }
});